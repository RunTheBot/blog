...
const FileDownload = ({ displayName, fileUrl }: { displayName: string; fileUrl: string }) => {
  ...
  return (
    <a
      href={fileUrl}
      download={fileName}
      className="flex max-w-sm items-center rounded-lg border-2 border-gray-900 bg-transparent p-3 text-primary-500 no-underline transition-all transition-all hover:text-primary-600 dark:hover:text-primary-400"
    >
